...
data PieceType = Pawn | Rook | Knight deriving (Show)
...
instance Show Board where
  show (Board brd bnds) = let width = maxX bnds - minX bnds + 1
                              ...
                              -- Generate a list of strings that alternate between '.' and 'O'
                              -- Each string represents a row on the chess board
                              textSquares =
                                [[if odd (x+y) then '.' else 'O' | x <- [1 .. width]] | y <- [1 .. height]]
...
-- Take mechanics for long range pieces
takeLong :: BoardMap -> Color -> [Square] -> [Square]
takeLong brd color sqrs = case takeUntil (nullSquare brd) sqrs of
                            [] -> []
                            xs -> takeWhile (not . sameColor brd color) xs
...
validMoves (Board brd bnds) (Piece Pawn color (Square col row)) =
  let (direction, initRow) = if color == White then (1,2) else (-1, 7)
      ...
      captureMoves = filter (not . sameColor brd color ) captureSquares
  in captureSquares
  
validMoves (Board brd bnds) (Piece Rook color (Square col row)) =
  let posX = [Square (col + x) row | x <- [1..maxX bnds - col]]
      ...
      -- validX = concat $ map (takeWhile $ nullSquare brd) $ [negX, posX, negY, posY]
      validX = concat $ map (takeLong brd color) $ [negX, posX, negY, posY]
  in validX
...
genBoard bnds = let whitePawns = [(Square x 2, Piece Pawn White (Square x 2)) | x <- [1..8]]
                    ...
                    rook = [(Square 5 5, Piece Rook White (Square 5 5))]
                in Board (HMap.fromList (whitePawns ++ blackPawns ++ rook)) bnds
